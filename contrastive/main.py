...
from contrastive.models import SiameseLSTM
from contrastive.similarityVectorizedDataset import SimilarityVectorizedDataset
...
if __name__ == "__main__":
    ...
    for i in range(EPOCHS):
        ...
        for local_batch, local_labels in dataloader_train:
            ...
            print(
                f"\r Epochs {i} - Loss: {total_loss / n} - Acc: {0} - Batch: {n}/{num_train_batch} - Dur: {total_duration}s/{estimated_duration_left}s",
                end="")
            n += 1
            t0 = time.time()
